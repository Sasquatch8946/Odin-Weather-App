...
const weatherApp = (function () {
  ...
  const parseData = function (data) {
    ...
    return {
      temp: processTemp(currentConditions.temp),
      conditions: currentConditions.conditions,
      feelslike: processTemp(currentConditions.feelslike),
  ...
  const getGIF = async function (searchTerm) {
    const response = await fetch(
      `https://api.giphy.com/v1/gifs/translate?api_key=${GIPHY_KEY}&s=${searchTerm}&weirdness=1`,
    );
    if (response.ok) {
